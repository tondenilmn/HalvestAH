/* static/live_model.js — E5 unified in-play pricing engine
 * ==========================================================================
 ...
 * depend on app.js's live-decay code, and nothing in app.js's Live Games
 * tab calls it yet (E7 wires the UI; E8 wires Telegram). Both old engines
 * keep working unchanged.
 *
 * ── Dual usage (browser + Node) ────────────────────────────────────────
 * Vanilla script, no ES modules. In Node: `require('../static/live_model.js')`
 * — it auto-loads `static/data/goal_hazard.json` and
 * `static/data/lambda_lookup.json` from disk on first use. In the browser:
 * `<script src="live_model.js">` exposes `window.LiveModel`; call
 * `LiveModel.init({hazard, lambdaLookup})` once with the two fetched JSON
 * blobs before pricing.
 *
 * Unlike `telegram/live_odds.js` (a hand-mirrored copy of app.js code, kept
 * in sync by hand), this file is meant to be REQUIRED directly from
 * telegram/ so there is exactly one copy and no drift. See the header note
 * in telegram/live_odds.js for the older mirroring convention.
 *
 * ── The model ──────────────────────────────────────────────────────────
 *
