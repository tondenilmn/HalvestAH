'use strict';
// ── AH Line Moves Toward Dog (shrinks) → Bet Fav AH Cover ─────────────────────
//
// Condition: closing AH line is ≥ 0.25 shallower than opening (fav_lo - fav_lc ≥ 0.20)
// i.e. underdog steamed — the market moved back toward level ball.
// Bet: favourite covers the AH at closing odds (fav_oc)
//
// P&L per unit staked (handles quarter-line half results):
//   ah2h = fav_2h - dog_2h - fav_line
//   ah2h >  0.26  → full fav win:  profit = fav_oc - 1
//   ah2h >  0.01  → half fav win:  profit = 0.5 * (fav_oc - 1)
//   |ah2h| ≤ 0.01 → push:          profit = 0
//   ah2h < -0.01  → half fav loss: profit = -0.5
//   ah2h < -0.26  → full fav loss: profit = -1
//
// Usage:
//   node backtest_favsteam.js          — TOP+MAJOR filter, at least 1 step (≥ 0.20)
//   node backtest_favsteam.js --all    — all leagues
//   node backtest_favsteam.js --exact  — exactly 1 step (0.20–0.30 only)
//   node backtest_favsteam.js --min2   — at least 2 steps (≥ 0.45)
...
const MODE_EXACT = process.argv.includes('--exact'); // exactly 1 step: 0.20–0.30
const MODE_MIN2  = process.argv.includes('--min2');  // at least 2 steps: ≥ 0.45
...
// "dog steam" = fav line shrank = fav_lo > fav_lc
function buildFilter() {
  if (MODE_EXACT) {
    // Exactly 1 step: movement between 0.20 and 0.30
    return r => r.fav_lo != null && (r.fav_lo - r.fav_lc) >= 0.20 && (r.fav_lo - r.fav_lc) <= 0.30;
  }
  if (MODE_MIN2) {
    // At least 2 steps: movement ≥ 0.45
    return r => r.fav_lo != null && (r.fav_lo - r.fav_lc) >= 0.45;
  }
  // Default: at least 1 step (≥ 0.20)
  return r => r.fav_lo != null && (r.fav_lo - r.fav_lc) >= 0.20;
}

// Compute P&L per unit for fav AH cover
function computePnl(r) {
  const ah2h = r.fav_2h - r.dog_2h - r.fav_line;
  if      (ah2h >  0.26) return r.fav_oc - 1;         // full fav win
  else if (ah2h >  0.01) return 0.5 * (r.fav_oc - 1); // half fav win (quarter line)
  else if (ah2h < -0.26) return -1;                    // full fav loss
  else if (ah2h < -0.01) return -0.5;                  // half fav loss (quarter line)
  else                   return 0;                     // exact push
}
...
// ── Main ───────────────────────────────────────────────────────────────────────
function main() {
  const modeLabel = MODE_EXACT ? 'exactly 1 step (0.20–0.30)'
                  : MODE_MIN2  ? 'at least 2 steps (≥ 0.45)'
                               : 'at least 1 step (≥ 0.20)';
  ...
  console.log(`DOG STEAM → FAV AH COVER BACKTEST  (tier=${LEAGUE_TIER}  ·  movement: ${modeLabel})`);
  console.log('  Condition: fav AH line shrank (underdog steamed toward level ball)');
  console.log('  Bet: favourite covers pre-match AH line at closing odds (fav_oc)');
  ...
  for (const { label, name } of MONTHS) {
    ...
    const anyWin  = (w + hw) / n * 100;
    const avgOdds = avg(matched.map(r => r.fav_oc));
    const roi     = pnl / n * 100;
    const avgMove = avg(matched.map(r => r.fav_lo - r.fav_lc));
  ...
  console.log('\n── Breakdown by closing AH line ──────────────────────────────────────────────');
  console.log('  fav_line    n       W    W%   AnyW%   Avg fav_oc    ROI%');
  ...
  for (const line of lines) {
    ...
    const gAvgOc = avg(group.map(r => r.fav_oc));
  ...
  // ── Sensitivity: effect of shrink size ───────────────────────────────────
  console.log('\n── Sensitivity: fav ROI by line shrink magnitude ─────────────────────────────');
  console.log('  Movement range      n       W    W%    ROI%    Avg fav_oc');
  ...
  for (const band of steamBands) {
    const group = fullDb.filter(r =>
      r.fav_lo != null &&
      (r.fav_lo - r.fav_lc) >= band.lo &&
      (r.fav_lo - r.fav_lc) <  band.hi
    );
