...
- **Live tab UI mode (added 2026-08-29, PLAN_FOCUS_BETS.md Phase 7)**: `state.liveUiMode` — `'focus'` (default) or `'all'`, set via `setLiveUiMode()`, persisted to `localStorage['halvest_live_ui']` (mode + whether the left panel's `#live-advanced` `<details>` is open). Focus mode shows one compact `.scan-card` per match with ONLY the 1T/2T panel (`renderFocusMarketsPanel`) — no Top Pick preview, no beta model ladder — plus a "🏆 BEST FOCUS BET" banner (best `rankScore` among the 7 `FOCUS_MARKET_BETS` keys only, via `topFocusBet`/`bestFocusEdgeScore`) and matches sorted by that same score. All-markets mode is the original, unchanged layout (Top Pick preview, beta model panel, "🏆 BEST LIVE BET" banner, "ALL QUALIFYING BETS" list). The left-panel controls not needed for Focus mode (beta model toggles, exchange commission, market-filter checkboxes) live behind one collapsed `⚙️ Advanced` `<details>` in both modes.
- `renderLiveGames()` — the mode-appropriate banner (see above) followed by one compact `.scan-card` per match (`renderLiveMatchCard`), each clickable by array index (`openLiveMatchDetail(idx)` — index into `_liveMatches`, deliberately **not** a team-name-derived id, since team/league names come from the scraped feed and aren't safe to embed raw into an inline `onclick`).
- `openLiveMatchDetail(idx)` opens `#live-match-modal` (new `.modal-overlay`/`.modal-box` CSS) showing the full per-match breakdown — reuses `renderMergedBetCard`/`renderTopValueBanner`/`renderValueHuntSection`/`buildTraceHtml`/`renderBetDashboard` unchanged, just fed this match's live-derived `cfg`/`gsForTrace` instead of manual input. In Focus mode these ("every market") sections are collapsed behind one "▸ Everything else" `<details>` below the 1T/2T panel; All-markets mode keeps them always-expanded, unchanged. Rank namespace `'live-detail-*'`.
