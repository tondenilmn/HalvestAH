...
## Dashboard L123-Style Consensus Near Kickoff (`static/app.js`, added 2026-08-27)

The Daily Dashboard's per-fixture pick (`analyzeFixtureForDashboard`) was opening-odds-only by design (`deriveOpeningContext`'s header comment) — no equivalent of `telegram/notify.js`'s Strategy L123 Layer 2 (movement) or Layer 3 (closing odds). For fixtures within `NEAR_KICKOFF_MIN` (30 min, the Dashboard's own narrowest window option) of kickoff, `l123ConsensusSignal` now also computes those two layers and combines them with Layer 1 (`openingOddsSignal`'s already cross-fit-corrected pick) the same way `runStrategyL123` does: each layer proposes a bet independently, and the fixture only gets a consensus if ≥2 of the layers that produced a rec land on the same bet key.

**Why 30 min, not L123's own 10-minute `PRE_MATCH_WINDOW_MIN`:** L123's Layer 3 was walk-forward validated against the true closing line taken right before kickoff. The Dashboard's wider windows (1h/4h/8h/24h) only have the live feed's *current* odds, which can still move a lot before real kickoff — treating that as "closing odds" further out would misrepresent a signal that was never validated at that horizon. 30 min is a conservative compromise specific to this integration, not the exact window notify.js itself fires on.

**`closingOddsSignal(favLine, favSide, favOc, tlC)`** mirrors `layer3Live`; **`movementSignal(moveCfg)`** (fed a `buildCfgFromLiveOdds`-built cfg) mirrors `layer2Live`. Both reuse the full `scoreBets`/`qualifiesBet` path (not the fast-scan `scoreBetsFast` cache Layer 1 uses for hundreds of fixtures), since consensus only ever runs for the handful of fixtures inside the 30-minute band — the per-fixture cost is the same as one Manual Analysis run, not the Dashboard's usual hundreds-of-fixtures scan.

When ≥2/3 layers agree, that bet replaces `sig1`'s opening-only pick as the fixture's headline bet, `qualifies` is forced true, and the fixture's tier is forced to `HIGH` regardless of the sig2/sig3 (market/timing) score — the L123 convergence signal is the same one `notify.js`'s own walk-forward validated, a stronger corroboration than either of the Dashboard's homegrown checks. The result's `l123` field (`{ bet, agreeCount, votes }`) is surfaced in the UI as a `🎯 L123 — N/3 signals agree (...)` line; fixtures outside the window, or inside it with no consensus, fall back to the original opening-odds-only scoring unchanged.
