...
**Not yet applied to:** the Dashboard (`runDailyDashboard`/`openingOddsSignal`/`scoreBetsFast`) — a differently-shaped, performance-tuned composite-signal architecture (cached per-pool baseline stats, hundreds of fixtures per run) that needs its own retrofit design rather than reusing `crossFitBets` directly. `telegram/engine.js`'s `processRow` also now stamps the same `fold` field (kept in sync per this file's sync-requirement note) but no Telegram strategy (L123/LATEGOAL/QUIET2H) consumes it yet — those use a differently-shaped selection path (`l123BestQualifying`, `lateGoalRec`, etc.), a separate follow-up.

**Cost:** halving the pool per fold means more cells fall under `MIN_N` — coverage (how often a match gets a qualifying pick at all) drops noticeably (backtested at roughly 5-9% fewer qualifying picks per month) in exchange for the ROI fix. Live Games' per-match analysis now runs `scoreBets` roughly 3x more often (once per fold, per stage) than before.
